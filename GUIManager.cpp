...
GUIManager::GUIManager(int width, int height) : mScreenWidth(width), mScreenHeight(height)
{}
...
std::shared_ptr<GUIManager> gGUIManager = nullptr;
...
bool GUIManager::InitWindows(HWND& mainWindow) {
	...
	mainWindow = CreateWindow(wc.lpszClassName, L"PBR Renderer",
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		wr.right - wr.left,
...
bool GUIManager::InitGUI(HWND& mainWindow, ComPtr<ID3D11Device>& device, ComPtr<ID3D11DeviceContext>& context) {
	IMGUI_CHECKVERSION();
	ImGui::CreateContext();
